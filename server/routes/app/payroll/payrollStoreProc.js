...
let GetEmployeesShiftAndConcepts = (type, from, to, payroll_Id, createdBy) => [ {$match: {
  $or: [
...
{$unwind: "$employeePosition"},
{
  ...
  $addFields: {
    ...
    totalCompassionate: {$toDecimal: {$sum: "$employeeCompassionate.amount"}},
  }
},
{
  $addFields: {
  ...
  $addFields: {
    ...
    grossPayment: {$toDecimal: {
      ...
      [
        ...
        "$totalOtherPays",
        "$totalTaxableBonus",
        "$totalMaternities", "$totalCSL", "$totalFinalPayments", "$totalCompassionate"
      ]
    }},
  }
},
...
let GetEmployeeHoursStats = (type, from, to) => [ // Stage 1
      {
        $match: {
            $or: [
              { status: "active", "payroll.payrollType": type },
...
let GetPayedPayroll = (id) => [
  ...
  { $sort: { fromDate: -1 } },
  {
    $group: {
      ...
      employeeCompassionate: { $push: "$employeeCompassionate" },
      employeeCSL: {$push:"$employeeCSL"},
      employeeTaxableBonus: {$push: "$employeeTaxableBonus"},
      employeeNonTaxableBonus: {$push: "$employeeNonTaxableBonus"},
      ...
      totalCompassionate: {$sum: "$totalCompassionate"},
