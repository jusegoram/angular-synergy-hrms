...
router.delete('/delete', (req, res) => {
  ...
  EmployeeCompany.deleteMany({
    employeeId: employeeId
  }, function (err, resp) {
    if (err) {
      res.status(500);
    } else {
      Shift.employeeShift.deleteMany({
        employeeId: employeeId
      }, function (err, resp) {
        if (err) {
          res.status(500);
          ...
          EmployeePosition.deleteMany({
            ...
            if (err) {
              ...
              EmployeePayroll.EmployeePayroll.deleteMany({
                employeeId: employeeId
              }, function (err, resp) {
                if (err) {
                  res.status(500);
                } else {
                  EmployeePersonal.deleteMany({
                    employeeId: employeeId
                  }, function (err, resp) {
                    if (err) {
                      res.status(500);
                    } else {
                      EmployeeFamily.deleteMany({
                        employeeId: employeeId
                      }, function (err, resp) {
                        if (err) {
                          res.status(500);
                        } else {
                          EmployeeComments.deleteMany({
                            employeeId: employeeId
                          }, function (err, resp) {
                            if (err) {
                              res.status(500);
