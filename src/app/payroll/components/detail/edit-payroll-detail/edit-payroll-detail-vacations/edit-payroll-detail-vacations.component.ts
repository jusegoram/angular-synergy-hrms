...
user: any;
  ...
  loadData() {
    ...
    this._payrollService.getConcepts({
      type: 'Other Payments',
  ...
  confirmValue(...args) {
    ...
    if (this.user.userId === rowItem.creationFingerprint) {
      ...
      this._payrollService.updateConcept({type: 'Other Payments', id: [rowItem._id], query: {
        verified: true,
        ...
        if (result) {
          ...
          .subscribe(finalResult => {
            this.successSwal.fire().then(e => console.log(e)).catch(e => console.log(e));
          });
      }
      });
    }
