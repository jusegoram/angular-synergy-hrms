...
export class ExportComponent implements OnInit {
  ...
  constructor(private payrollService: PayrollService, private fb: FormBuilder) {
    this.clients = [];
    this.campaigns = [];
    this.status = this.payrollService.status;
  }
  ngOnInit() {
    this.payrollService.getClient().subscribe(data => this.clients = data);
    this.buildForm();
  }
  buildForm() {
  ...
  getReport() {
    ...
    this.payrollService.getReport(obj).subscribe(
        data => {
          let filtered: any;
          filtered = data;
  ...
  export() {
    ...
    XLSX.utils.book_append_sheet(wb, shift, 'shift');
    XLSX.utils.book_append_sheet(wb, position, 'position');
    XLSX.utils.book_append_sheet(wb, personal, 'personal');
    XLSX.utils.book_append_sheet(wb, family, 'family');
    /* save to file */
    XLSX.writeFile(wb, 'export-payroll-info.xlsx');
  }
